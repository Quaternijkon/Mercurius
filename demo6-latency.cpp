...
int main() {
    ...
    // 3. Build Loop
    cout << "Starting Unified Build..." << endl;
    ...
    // We measure HNSW search time. Since it is batched, we assign average time to all queries or simulate per-query.
    // For Demo purpose, we run 1-by-1 to get accurate latency distribution, OR we run batch and average.
    // To match demo4 structure (batch), we timestamp around batch.
    // But `demo6` is latency demo, let's try to be precise.
    // However, the code below keeps batch structure to align with demo4 exactly.
    
    double t_hnsw_start = get_time_ms();
    index_hnsw->search(nq, xq, SEARCH_EF, hnsw_D.data(), hnsw_I.data());
    double t_hnsw_end = get_time_ms();
    double avg_hnsw_us = (t_hnsw_end - t_hnsw_start) * 1000.0 / nq;

    // Analyze & IVF Search Prep (Step 2)
    double t_prep_start = get_time_ms();
    idx_t centroid_mask = 1ULL << 60;
    ...
    double t_prep_end = get_time_ms();
    double avg_prep_us = (t_prep_end - t_prep_start) * 1000.0 / nq;

    // IVF Search (Step 3) - Use search_preassigned_stats
    vector<float> ivf_D(nq * SEARCH_K);
    vector<idx_t> ivf_I(nq * SEARCH_K);
    IVFSearchParameters ivf_params;
    ivf_params.nprobe = max_clusters;
    
    // We expect per-query stats here
    // Note: search_preassigned_stats expects parallel_mode=0 for individual stats
    int old_pmode = index_ivf->parallel_mode;
    index_ivf->parallel_mode = 0; 
    index_ivf->search_preassigned_stats(nq, xq, SEARCH_K, ivf_assign_query.data(), ivf_centroid_dis.data(), 
                                        ivf_D.data(), ivf_I.data(), false, &ivf_params, nullptr, stats.data());
    index_ivf->parallel_mode = old_pmode;

    // Fill Quantization stats (HNSW + Prep)
    for(size_t i=0; i<nq; ++i) {
        stats[i].quantization_us = avg_hnsw_us + avg_prep_us;
        stats[i].total_us = stats[i].quantization_us + stats[i].list_scan_us;
    }
    ...
    // Stats Output
    double total_quant = 0, total_scan = 0, total_total = 0;
    for (size_t i = 0; i < 5; ++i) {
        cout << "Query " << i << ": Quant=" << stats[i].quantization_us << "us, Scan=" << stats[i].list_scan_us << "us, Total=" << stats[i].total_us << "us" << endl;
    }
    for (size_t i = 0; i < nq; ++i) {
        total_quant += stats[i].quantization_us;
        total_scan += stats[i].list_scan_us;
        total_total += stats[i].total_us;
    }

    cout << "------------------------------------------------" << endl;
    cout << "Average Latency (over " << nq << " queries):" << endl;
    cout << "Quantization (HNSW+Prep): " << total_quant / nq << " us" << endl;
    cout << "List Scan (IVF):          " << total_scan / nq << " us" << endl;
    cout << "Total:                    " << total_total / nq << " us" << endl;
