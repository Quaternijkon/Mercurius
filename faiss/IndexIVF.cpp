...
struct Timer {
    ...
    double elapsed_us() {
        ...
        return (now.tv_sec - start.tv_sec) * 1000000.0 + (now.tv_usec - start.tv_usec);
    }
};
}
...
void IndexIVF::search_stats(
    ...
    FAISS_THROW_IF_NOT(k > 0);
    // MODIFICATION #1: (Recommended) Clear stats array at start
    if (per_query_stats) {
    ...
    auto sub_search_func = [this, k, nprobe, params](
        ...
        // MODIFICATION #4: Add timer for coarse quantization
...
#pragma omp parallel for if (nt > 1)
        for (idx_t slice = 0; slice < nt; slice++) {
            ...
            if (i1 > i0) {
                try {
                    // MODIFICATION #5: Pass per_query_stats with offset
                    sub_search_func(
                            i1 - i0,
                            x + i0 * d,
                            distances + i0 * k,
        ...
        // MODIFICATION #5 (else branch): Pass per_query_stats pointer
        sub_search_func(n, x, distances, labels, &indexIVF_stats, per_query_stats);
    }

    // MODIFICATION #6: Calculate total time
...
#pragma omp for
            for (idx_t i = 0; i < n; i++) {
                // START OF MODIFICATION #1: Declare timer
                ...
                // START OF MODIFICATION #2: Record time
                if (list_scan_timer) { 
